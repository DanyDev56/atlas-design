...
namespace Atlas\Modules\Billing\Infrastructure;

use Illuminate\Support\Facades\DB;

final class PostgresBillingIdempotencyStore
{
    /** @return array<string, mixed>|null */
    public function find(string $scope, string $key): ?array
    {
        $row = DB::table('billing.idempotency_keys')
            ->where('scope', $scope)
            ->where('key', $key)
            ->first();
    ...
    {
        DB::table('billing.idempotency_keys')->insert([
            'scope' => $scope,
            'key' => $key,
