...
use Atlas\Modules\Crm\Domain\ClientBillingProfileUpdated;
...
final class UpdateClientBillingProfileHandler
{
    ...
    public function handle(
        ...
        $this->authorizer->authorize($actorUserId, $workspaceId, 'crm.clients.update-billing-profile');

        ksort($billingProfile);
        $scope = 'crm.update_client_billing_profile';
        $fingerprint = hash('sha256', json_encode([
            $workspaceId, $clientId, $billingProfile, $expectedRevision,
        ], JSON_THROW_ON_ERROR));
        ...
        return DB::transaction(function () use (
            ...
            $client->updateBillingProfile($billingProfile, $now);
            $this->clients->update($client);

            $event = new ClientBillingProfileUpdated(
                clientId: new ClientId($clientId),
                workspaceId: $workspaceId,
                billingProfileVersion: $client->billingProfileVersion(),
