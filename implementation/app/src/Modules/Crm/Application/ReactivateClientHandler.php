...
use Atlas\Modules\Crm\Domain\ClientReactivated;
...
final class ReactivateClientHandler
{
    ...
    public function handle(
        ...
        $this->authorizer->authorize($actorUserId, $workspaceId, 'crm.clients.reactivate');

        $scope = 'crm.reactivate_client';
        $fingerprint = hash('sha256', json_encode([
            $workspaceId, $clientId, $expectedRevision,
        ], JSON_THROW_ON_ERROR));
        ...
        return DB::transaction(function () use (
            ...
            $client->reactivate($now);
            $this->clients->update($client);

            $event = new ClientReactivated(
