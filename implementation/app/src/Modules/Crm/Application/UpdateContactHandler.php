...
use Atlas\Modules\Crm\Domain\ContactUpdated;
...
final class UpdateContactHandler
{
    ...
    public function handle(
        ...
        $this->authorizer->authorize($actorUserId, $workspaceId, 'crm.contacts.update');

        $scope = 'crm.update_contact';
        $fingerprint = hash('sha256', json_encode([
            $workspaceId, $clientId, $contactId, $profileChanges, $expectedRevision,
        ], JSON_THROW_ON_ERROR));
        ...
        return DB::transaction(function () use (
            ...
            $contact->updateProfile($profileChanges);
            $now = new \DateTimeImmutable('now', new \DateTimeZone('UTC'));
            $client->recordContactUpdated($now);
