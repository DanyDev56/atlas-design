...
namespace Atlas\Modules\Workspace\Infrastructure;

use Illuminate\Support\Facades\DB;

final class PostgresWorkspaceIdempotencyStore
{
    /** @return array<string, mixed>|null */
    public function find(string $scope, string $key): ?array
    {
        $row = DB::table('workspace.idempotency_keys')
            ->where('scope', $scope)
            ->where('key', $key)
            ->first();
    ...
    {
        DB::table('workspace.idempotency_keys')->insert([
            'scope' => $scope,
            'key' => $key,
