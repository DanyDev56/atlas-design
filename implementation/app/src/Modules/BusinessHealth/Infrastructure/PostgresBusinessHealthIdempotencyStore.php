...
namespace Atlas\Modules\BusinessHealth\Infrastructure;

use Illuminate\Support\Facades\DB;

final class PostgresBusinessHealthIdempotencyStore
{
    /** @return array<string, mixed>|null */
    public function find(string $scope, string $key): ?array
    {
        $row = DB::table('business_health.idempotency_keys')
            ->where('scope', $scope)
            ->where('key', $key)
            ->first();
    ...
    {
        DB::table('business_health.idempotency_keys')->insert([
            'scope' => $scope,
            'key' => $key,
