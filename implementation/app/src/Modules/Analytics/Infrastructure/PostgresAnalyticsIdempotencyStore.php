...
namespace Atlas\Modules\Analytics\Infrastructure;

use Illuminate\Support\Facades\DB;

final class PostgresAnalyticsIdempotencyStore
{
    /** @return array<string, mixed>|null */
    public function find(string $scope, string $key): ?array
    {
        $row = DB::table('analytics.idempotency_keys')
            ->where('scope', $scope)
            ->where('key', $key)
            ->first();
    ...
    {
        DB::table('analytics.idempotency_keys')->insert([
            'scope' => $scope,
            'key' => $key,
